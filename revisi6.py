...
class Perpustakaan:
    ...
    def tampilkan_buku(self):
        ...
        if buku_tersedia:
            buku_terurut = sorted(buku_tersedia, key=lambda buku: buku.judul)
            return "\n".join(str(buku) for buku in buku_terurut)
    ...
    def tampilkan_anggota(self):
        if self.anggota_perpustakaan:
            anggota_terurut = sorted(self.anggota_perpustakaan, key=lambda anggota: anggota.nama)
            return "\n".join(f"{anggota.nama}, ID: {anggota.id_anggota}" for anggota in anggota_terurut)
